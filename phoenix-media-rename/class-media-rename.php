...
class Phoenix_Media_Rename {
	...
	function ajax_pnx_rename() {
		if (check_ajax_referer('phoenix_media_rename', '_wpnonce', 0)) {
			...
			if (!current_user_can('edit_post', $attachment_id)){
				...
				console.log(__("user doesn't have permission to edit the post", constant('PHOENIX_MEDIA_RENAME_TEXT_DOMAIN')));
	...
	static function do_rename($attachment_id, $new_filename, $retitle = 0, $title_from_post = 0, $name_from_post = 0, $check_post_parent = true ) {
		...
		$option_sanitize_filename = self::get_option_boolean( $options, 'pmr_sanitize_filenames', true );
				...
				foreach ($metas as $key => $meta) {
					switch ($key){
						...
						case '_elementor_data':
							...
							if ( self::is_plugin_active( constant( "pluginElementor" ) ) ) {
								$table_name = $wpdb->prefix . 'postmeta';
								try{
