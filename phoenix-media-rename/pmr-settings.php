...
class PmrSettingsPage
{
	...
	{
		print __( 'Check to sanitize file name, uncheck to leave filename as entered by user:<br><strong>Please Note</strong>: disabling this option can generate filenames that are incompatible with the server, disable at your own risk!', constant( 'PHOENIX_MEDIA_RENAME_TEXT_DOMAIN' ) );
	}
	...
	{
		echo '<input type="checkbox" id="pmr_sanitize_filenames" name="pmr_options[pmr_sanitize_filenames]" value="1" ' . checked(1, isset( $this->options['pmr_sanitize_filenames'] ) ? esc_attr( $this->options['pmr_sanitize_filenames']) : 1, false) . '//>';
	}
