...
class WoocommerceIR_SMS_Gateways {
	...
	public function newsms() {
		...
		$content = 'http://newsms.ir/api/?action=SMS_SEND&username=' . rawurlencode( $username ) .
		           '&password=' . rawurlencode( $password ) .
		           '&API_CHANGE_ALLOW=true&to=' . rawurlencode( $to ) .
		           '&api=1&from=' . rawurlencode( $from ) .
		           '&FLASH=0&text=' . rawurlencode( $massage );
	...
	public function sahandsms() {
		...
		$ch = curl_init( 'http://webservice.sahandsms.com/NewSMSWebService.asmx/SendFromUrl?username=' . $username . '&password=' . $password . '&fromNumber=' . $from . '&toNumber=' . $to . '&message=' . urlencode( trim( $massage ) ) );
		curl_setopt( $ch, CURLOPT_HTTPHEADER, array(
			'Accept: text/html',
			'Connection: Keep-Alive',
			'Content-type: application/x-www-form-urlencoded;charset=UTF-8'
	...
	public function aryana() {
		...
		$ch = curl_init( 'http://www.payamkotah.ir/FastSendSMS.ashx?user='. $username .'&pass='.$password.'&line='.$from.'&flash=1&mobile='. $to .'&message='. $massage);
		curl_setopt( $ch, CURLOPT_HTTPHEADER, array(
			'Accept: text/html',
			'Connection: Keep-Alive',
			'Content-type: application/x-www-form-urlencoded;charset=UTF-8'
	...
	public function npsms() {
		...
		$content = 'https://npsms.com/sendSmsViaURL.aspx?domainName=npsms.com&userName=' . rawurlencode( $username ) .
		           '&password=' . rawurlencode( $password ) .
		           '&reciverNumber=' . rawurlencode( $to ) .
		           '&senderNumber=' . rawurlencode( $from ) .
		           '&smsText=' . rawurlencode( $massage );
		...
		if ( ! empty( $sms_response ) && $sms_response >= 1 ) {
			return $response = true;//success
		} else {
			$response = $sms_response;
		}
